...
function setup() {
  ...
  // Init globals
  for (let i = 0; i < T_TYPES.length; i++) T_TYPES[i].color = color(T_TYPES[i].colorHex);
  INPUT = new Input();
...
function generateTerrain(type) {
  ...
  if (type == 0) {
    for (let x = 0; x < T_GRID.gridWidth; x++) {
      let grassStart = T_GRID.gridHeight * 0.4 + floor(noise(x * 0.02) * T_GRID.gridHeight * 0.4);
      let dirtStart = grassStart + 2 + floor(noise(x * 0.3, 100) * 4);
      let stoneStart = dirtStart + 10 + floor(noise(x * 0.3, 200) * 5);

      for (let y = grassStart; y < dirtStart; y++) T_GRID.setCell(x, y, "grass");
      for (let y = dirtStart; y < stoneStart; y++) T_GRID.setCell(x, y, "dirt");
      for (let y = stoneStart; y < T_GRID.gridHeight; y++) T_GRID.setCell(x, y, "stone");
    }
...
class TerrainGrid {
  ...
  setCell(x, y, type) {
    ...
    if (chunkX < 0 || chunkX > this.chunkCountX || chunkY < 0 || chunkY > this.chunkCountY) return;
    this.chunks[chunkX][chunkY].setCell(x - chunkX * this.chunkSize, y - chunkY * this.chunkSize, type);
  }
  ...
  getCell(x, y) {
    ...
    let chunkY = floor(y / this.chunkSize);
    if (chunkX < 0 || chunkX > this.chunkCountX || chunkY < 0 || chunkY > this.chunkCountY) return;
    this.chunks[chunkX][chunkY].getCell(x - chunkX * this.chunkSize, y - chunkY * this.chunkSize);
  };
}
