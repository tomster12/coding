...
class GenAlgorithm {
  ...
  void updateGeneration() { // Called each frame, Shows and updates
    if (generated) {
      if (ready && toUpdate) singleUpdateGeneration();
      else if (!ready && toFinish) finishGeneration();
    }
  }
  ...
  void showGeneration() { // Show all units
    ...
    for (int i = 0; i < population.size(); i++) {
      ...
      if (population.get(i).ontop()) toShow = population.get(i);
    }

    if (toShow != null) {
      netDrawer = new NetDrawer(toShow.net, new PVector(0, 200), new PVector(200, 200), 10);
      toShow.updateFitness();
      fill(0);
      text(toShow.fitness, 65, 150);
    }
  ...
  void fitnessGeneration() { // Update all units fitness
    ...
    bestFitness = 0; // Reset best each generation
    ...
    bestDrawer = new NetDrawer(bestNet, new PVector(0, 400), new PVector(200, 200), 10);
  }
