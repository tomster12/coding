...
app.post("/sensor-data", (req, res) =>{
  ...
  console.log("Received data: " + req.body.sensor_name + " = " + req.body.sensor_data);
  const sensorData = new SensorData({ sensor_name: req.body.sensor_name, sensor_data: req.body.sensor_data });
  sensorData.save()
  .then(() => {
    res.sendStatus(200);
  })
...
app.get("/sensor-data", (req, res) => {
  ...
  .then((groupedData) => {
    const data = groupedData.reduce((acc, group) => { acc[group._id] = group.docs; return acc; }, { });
    res.json(data);
  })
