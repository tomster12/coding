...
// Setup types
type MOUSEBUTTON = 0 | typeof LEFT | typeof CENTER | typeof RIGHT;
type Entity = Player;

// Declare input variables
let inputs: {[propName: number]: boolean};
let inputsPressed: {[propName: number]: boolean};
let mouseInput: MOUSEBUTTON;
let mouseInputPressed: MOUSEBUTTON;
let mouseInputPressedLocked: boolean;

// Declare main variables
let groundWeapons: Weapon[];
...
function draw() {
  ...
  for (let weapon of groundWeapons) weapon.update();
  for (let projectile of projectiles) projectile.update();
  player.update();
  ...
  player.translateTo();
  for (let projectile of projectiles) projectile.show();
  for (let weapon of groundWeapons) weapon.show();
  player.show();
...
function getScreenMousePos(): Vec2 {
  // Returns a vector representing the mouse position
  return new Vec2(
    mouseX - player.getTranslateX(),
    mouseY - player.getTranslateY()
  );
}
