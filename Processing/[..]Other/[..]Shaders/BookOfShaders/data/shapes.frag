...
vec3 draw(vec3 col0, vec3 col1, float pct, bool add=false) {
  if (pct == 0) return col0;
...
void main() {
  ...
  color = draw(color, vec3(0.93, 0.25, 0.43), step(0, -circle1));

  // Rect 1
	float rect1 = rectangle(st, vec2(0.2, 0.5), vec2(0.6, 0.85));
  color = draw(color, vec3(0.25, 0.41, 1.0), step(0, -rect1), true);

  // Rect 2
	float rect2 = rectDist(st, vec2(0.35, 0.42), vec2(0.88, 0.62));
	color = draw(color, vec3(0.64, 0.17, 0.92), step(0, -rect2));

  // Circles 2,1 and 2,2
	float circle21 = circleDist(st, vec2(0.72, 0.24), 0.22 + sin(u_time) * 0.02);
	float circle22 = circleDist(st, vec2(0.37, 0.19), 0.15);
  color = draw(color, vec3(0.72, 0.88, 0.33), step(0, -smin(circle21, circle22, 0.3)));

  // Circles 3,1 and 3,2
	float circle31 = circleDist(st, vec2(0.78 + sin(u_time * 1.4) * 0.04, 0.85), 0.08);
  float circle32 = circleDist(st, vec2(0.87, 0.76), 0.12);
  color = draw(color, vec3(0.13, 0.29, 0.82), step(0, -circle31));
	color = draw(color, vec3(0.13, 0.29, 0.82), step(0, -circle32));
	color = draw(color, vec3(0.21, 0.73, 0.34), step(0, -max(circle31, circle32)));
