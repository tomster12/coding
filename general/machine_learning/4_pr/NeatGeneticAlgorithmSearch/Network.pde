...
class Network {
  ...
  boolean mutateWeight() {
    ...
    if (connections.size() > 0) {
      int r = floor(random(connections.size()));
      if (random(1) < 0.2) {
        connections.get(r).enabled = !connections.get(r).enabled;
      } else {
  ...
  Network crossover(Network otherNet) {
    ...
    for (int i = 0; i < connections.size(); i++) { // Get possible connections
  ...
  float speciateDistance(Network oNet) {
    ...
    for (int i = 0; i < connections.size(); i++) { // For each connection
      ...
      int cCInno = cc.cInno;
      Connection oc = getCInno(oNet.connections, cCInno);
      if (oc == null) { // If other doesnt have it then ex++, otherwise change average weight
        ex++;
      } else {
        wa += abs(oc.weight - cc.weight);
        wc++;
      }
    }

    for (int i = 0; i < oNet.connections.size(); i++) { // For each connection in other net
      Connection cc = oNet.connections.get(i);
      int cCInno = cc.cInno;
      Connection oc = getCInno(connections, cCInno);
      if (oc == null) {
        ex++;
      } else {
        wa += abs(oc.weight - cc.weight);
