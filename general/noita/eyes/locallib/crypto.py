...
class JosseCipher:
    ...
    def encrypt_mv(self, msg_mv):
        ...
        for i in range(len(msg_mv)):
            if i == 0:
                msg_cv.append(self.modA(-self.S[msg_mv[i]] - 2))
            elif i == 1:
                msg_cv.append(self.modA(self.S[msg_mv[i]] - msg_cv[i - 1] - 1))
            else:
                msg_cv.append(self.modA(self.S[msg_mv[i]] + msg_cv[i - 1] + 1))
    ...
    def decrypt_cv(self, msg_cv):
        ...
        for i in range(len(msg_cv)):
            if i == 0:
                msg_mv.append(self.S_I[self.modA(-self.S[msg_cv[i]] - 2)])
            elif i == 1:
                msg_mv.append(
                    self.S_I[self.modA(self.S[msg_cv[i]] + self.S[msg_cv[i - 1]] + 1)])
            else:
                msg_mv.append(
                    self.S_I[self.modA(self.S[msg_cv[i]] - self.S[msg_cv[i - 1]] - 1)])
    ...
    def log(self):
        ...
        if self.key_wrapped != None:
            print("Key (S) Wrapped: ", self.key_wrapped)
        if self.S != None:
            print("Key (S): ", self.S)
        if self.S_I != None:
            print("Key (S) Inverse: ", self.S_I)
        if self.S != None:
            print("".join([self.map_v_to_a[a] for a in self.S]))
        if self.S != None:
            print("".join([self.map_v_to_a[self.S[i]]
...
class RotatingHomophonicCipher:
    def __init__(self, pt_a):
        ...
        for l in pt_a:
            ...
            self.buckets[l] = [current_bucket_end,
                               current_bucket_end + size, -1]
    ...
    def encode(self, pt):
        ...
        for l in self.buckets:
            self.buckets[l][2] = self.buckets[l][0]
