...
# For each repeated CT letter (l0 -> l1)
count = 0
for (msg_i, msg) in enumerate(eyes):
    for (l0_i, l0) in enumerate(msg):
        for l1_i in range(l0_i + 1, len(msg)):
            l1 = msg[l1_i]
            if l0 == l1:
                gap = l1_i - l0_i

                # For each other instance of that letter (l2)
                for (msg2_i, msg2) in enumerate(eyes):
                    for (l2_i, l2) in enumerate(msg2):
                        if (len(msg2) > (l2_i + gap)) and (msg2[l2_i + gap] != l2):
                            count += 1


end = time.perf_counter()
print(f"{((end - start) * 1_000):.2f}ms")
print(count)
