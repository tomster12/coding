	...
	for iso in isos:
		for ri in range(len(iso[2])):
			...
			for i in range(len(iso[0])):
				v = count if not use_pattern_value else (0 if np.isnan(iso[0][i]) else iso[0][i])
				im[ri][r[i]] = v
		count += 1
	return im
		...
		for y in range(len(labels)):
			for x in range(len(labels[y])):
				if not cast_labels or not math.isnan(labels[y][x]):
					...
					plt.text(x, y, label, ha="center", va="center", fontsize="7", alpha=(dull_amount if im[y][x] < under_value and to_dull else 0.9))
