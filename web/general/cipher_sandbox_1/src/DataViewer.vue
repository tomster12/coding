...
<div class="data-viewer">
    <div class="button toggle" @click="onToggleButtonClick">
        <div :class="['icon', { selected: isEditing}]"><img src="./assets/raw.png" alt="Edit icons created by Becris - Flaticon" /></div>
        <div :class="['icon', { selected: !isEditing}]"><img src="./assets/grid.png" alt="Menu icons created by Freepik - Flaticon" /></div>
    </div>
    ...
    <div  class="content">
        <textarea
            ...
            class="raw"
            ref="rawContent"
            :value="data.rawString"
            @input="onRawContentInput($event)"
        />

        <div class="grid" v-if="!isEditing">
            <div v-for="message in data.messages" class="message-grid">
                <div
                    v-for="letter in message" class="message-grid-item"
                    :style="{ backgroundColor: `rgb(${255*parseFloat(letter)/data.alphabetSize},${255*parseFloat(letter)/data.alphabetSize},${255*parseFloat(letter)/data.alphabetSize})` }"
                >{{ letter }}</div>
            </div>
        </div>
    </div>

    <div class="button submit" @click="onSubmitButtonClick">
        <img class="icon" src="./assets/submit.png" alt="Next icons created by Pixel perfect - Flaticon" />
    </div>
</div>
</template>
...
const rawContent = ref<HTMLInputElement | null>(null);
...
setDataRawString(eyesString);

watch(rawContent, () => {
    if (rawContent.value) {
        fitInputElement(rawContent.value as HTMLInputElement);
        fitInputElement(rawContent.value as HTMLInputElement);
    }
})
...
function setDataRawString(rawString: string) {
    const lines = rawString.split("\n").filter(l => l.length != 0);
    const messages = lines.map(line => line.split(/[, ]+/).filter(v => v != ""));
    const messageLengths = messages.map(message => message.length);
    let alphabetSize = 0;
    messages.forEach(message => message.forEach(l => alphabetSize = Math.max(alphabetSize, parseFloat(l))));
    data.rawString = rawString;
    data.messages = messages;
    data.messageLengths = messageLengths;
    data.alphabetSize = alphabetSize;
}

const setDataRawString_Throttled = debounce(setDataRawString, 100);
...
function onRawContentInput(event: Event) {
    setDataRawString_Throttled((event.target as HTMLInputElement).value);
    fitInputElement(rawContent.value as HTMLInputElement);
}
...
function fitInputElement(el: HTMLInputElement) {
    el.style.height = "0px";
