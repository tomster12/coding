class network {
  ...
  Mutate() {
    ...
    if (r < mWt) {
      this.connections[floor(random(0, this.connections.length))].weight = this.RandomWeight();

    } else if (r < mWt + mC) {
      this.AddConnection();

    } else {
      this.AddNeuron(this.connections[floor(random(0, this.connections.length))].inNo);
    }
  }
}
